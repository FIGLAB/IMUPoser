...
import numpy as np
import torch
# from pytorch3d.transforms import matrix_to_axis_angle # this is faster, but pytorch3d can be hard to install
...
def angle_between(rot1: torch.Tensor, rot2: torch.Tensor, rep=RotationRepresentation.ROTATION_MATRIX):
    ...
    # angles = matrix_to_axis_angle(offsets).norm(dim=1) # if using pytorch3d
    return angles
...
def rotation_matrix_to_r6d(r: torch.Tensor):
    ...
    """
    return r.reshape(-1, 3, 3)[:, :, :2].transpose(1, 2).clone().reshape(-1, 6)
